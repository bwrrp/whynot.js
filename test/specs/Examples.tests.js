		...
		describe('whynot.js examples', function() {
			...
			describe('regular expressions', function() {
				...
				it('can perform simple matching', function() {
					...
					chai.expect(vm.execute(createInput('abcdf')).length).to.equal(1);
					// But it won't match the string 'abcf'
					chai.expect(vm.execute(createInput('abcf')).length).to.equal(0);
				});
				...
				it('can complete a string based on a regex', function() {
					...
					// results:
					chai.expect(flattenRecordStrings(vm.execute(createInput('ad')))).to.deep.equal(['ade', 'adf']);
					// Fixing both choices yields only a single result:
					chai.expect(flattenRecordStrings(vm.execute(createInput('bf')))).to.deep.equal(['bcdf']);
					// While leaving both open generates all strings accepted by the regex:
					chai.expect(flattenRecordStrings(vm.execute(createInput('d')))).to.deep.equal(['ade', 'bcde', 'adf', 'bcdf']);
					// Finally, presenting an input which can not be made to match by adding
					// characters yields no results:
					chai.expect(flattenRecordStrings(vm.execute(createInput('abc')))).to.deep.equal([]);
				});
			});
		});
	}
